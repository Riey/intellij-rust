...
class ExprUseWalker(private val delegate: Delegate, private val mc: MemoryCategorizationContext) {
        ...
        when (expr) {
            is RsUnaryExpr -> {
                ...
                when {
                    ...
                    else -> consumeExpr(base) // `-foo`, `!foo`, and so on
                }
            }
    ...
    private fun walkMacroCall(macroCall: RsMacroCall) {
        ...
        macroCall.assertMacroArgument?.expr?.let(::walkExpr)
        macroCall.exprMacroArgument?.expr?.let(::walkExpr)
        macroCall.includeMacroArgument?.expr?.let(::walkExpr)
        macroCall.logMacroArgument?.expr?.let(::walkExpr)

        macroCall.concatMacroArgument?.exprList?.forEach(::walkExpr)
        macroCall.envMacroArgument?.exprList?.forEach(::walkExpr)
        macroCall.vecMacroArgument?.exprList?.forEach(::walkExpr)
        macroCall.formatMacroArgument?.formatMacroArgList?.map { it.expr }?.forEach(::walkExpr)
        macroCall.assertMacroArgument?.formatMacroArgList?.map { it.expr }?.forEach(::walkExpr)
    }
